...
void RenderInterface()
{
	...
	AliceUI::CreateInstance("Project Alice", nullptr);
	{
		...
		AliceUI::FormattedText<std::string>("Current API: %s", DataManager::get_api().c_str());

		if (DataManager::get_api() == DIRECTX11_API)
		{
			AliceUI::FormattedText<float>("Elapsed Time: %.1f", dx11_api_window.GetElapsedTime());
		}
		else if (DataManager::get_api() == OPENGL_API)
		{
...
auto APIENTRY WinMain(_In_ HINSTANCE instance, _In_opt_ HINSTANCE prev_instance, _In_ LPSTR cmd_line, _In_ int cmd_show) -> INT
{
	...
	// OpenGL API Window
	if (Globals::CheckAPI() == Alice::API::RendererAPI::OpenGL)
	{
	...
	if (Globals::CheckAPI() == Alice::API::RendererAPI::DirectX)
	{
